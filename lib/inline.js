...
export default function makeJuiceClient(juiceClient) {
  ...
  function inlineDocument($, css, options) {
    ...
    function handleRule(rule) {
      ...
      const parsedSelector = selector.parsed();

      if (!parsedSelector) {
        return;
      }

      const pseudoElementType = getPseudoElementType(parsedSelector);

      // skip rule if the selector has any pseudos which are ignored
      for (let i = 0; i < parsedSelector.length; ++i) {
        const subSel = parsedSelector[i];
        if (subSel.pseudos) {
          for (let j = 0; j < subSel.pseudos.length; ++j) {
            const subSelPseudo = subSel.pseudos[j];
      ...
      if (pseudoElementType) {
        const last = parsedSelector[parsedSelector.length - 1];
        const pseudos = last.pseudos;
  ...
  // Return "before" or "after" if the given selector is a pseudo element (e.g., a::after)
  function getPseudoElementType(selector) {
    if (selector.length === 0) {
      return;
    }

    const pseudos = selector[selector.length - 1].pseudos;
    if (!pseudos) {
      return;
    }
