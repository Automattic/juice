...
function inlineDocument($, css, options) {
  ...
  if(options.onlyMatchedOnce) {
    updateStyles($, removedRules, options);
  }
  ...
  function handleRule(rule) {
    ...
    // optionally only inline selectors that match only once
    if(options.onlyMatchedOnce) {
      // skip inlining selector that maches more than once
      if(els.length > 1) {
        return;
      }
...
function updateStyles($, removedRules, options) {
  var stylesList = $('style');
  var styleDataList, styleElement;
  stylesList.each(function() {
    ...
    var ast      = mensch.parse(styleElement.childNodes[0].nodeValue, {position: true, comments: true});
    var rulesAst = ast.stylesheet.rules;

    var changed  = false;
    for(var ruleAst of rulesAst) {
loop2:
      for(var removedRule of removedRules) {
        var removedRuleSelector     = removedRule[0];
        var removedRuleDeclarations = removedRule[1];
        if(ruleAst.selectors.indexOf(removedRuleSelector) !== -1 && 
           equal(removedRuleDeclarations, ruleAst.declarations)) {
          // remove rule
          ruleAst.selectors.splice(0, 1);
          changed = true;
        }

        if(ruleAst.selectors.length == 0) {
          // clean up rules with no selectors left
          rulesAst.splice(rulesAst.indexOf(ruleAst), 1);
          break loop2;
        }
      }
    }
    ...
    if(rulesAst.length == 0) {
      // remove style elements that became empty now
      $(styleElement).remove();
      return;
    }

    var newCss = mensch.stringify(
      ast,
      { comments: false, indentation: '  ' }
    );
    styleElement.childNodes[0].nodeValue = newCss;
