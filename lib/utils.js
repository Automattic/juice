...
// Every entry point reports a non-string input the same way, instead of
// failing later with an unrelated error from the parser.
export function htmlTypeError(html) {
  ...
  if (html === null) {
    ...
    type = (html.constructor && html.constructor.name) || 'object';
  }

  const hint = html instanceof Uint8Array
    ? ' If you read it from a file, pass \'utf8\' as the encoding or call .toString() on it.'
    : '';

  return new TypeError('juice expects an HTML string, got ' + type + '.' + hint);
}

// Adds !important to every declaration in rules that use one of the given
// pseudo-classes, so they can win over inlined styles (e.g. :hover). Parsed
// without the nesting plugin, so the rest of the CSS is written back as-is.
export function addImportantToPseudoClasses(css, pseudoClasses) {
  const names = pseudoClasses.map((name) => name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
  const pseudoRe = new RegExp('(^|[^:]):(' + names.join('|') + ')(?![\\w-])', 'i');
  const root = safeParser(css);
  let changed = false;

  root.walkRules((rule) => {
    if (!rule.selectors.some((selector) => pseudoRe.test(selector))) {
      return;
    }
    rule.each((node) => {
...
export function getDefaultOptions(options) {
  const result = Object.assign({
    ...
    addImportantToPseudoClasses: false,
    url: ''
  }, options);
