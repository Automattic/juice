...
var fs = require('fs');
var Batch = require('batch'); //TODO: remove, replace with pend
...
function getStylesheetList(document, options) {
  ...
  for (i = 0, length1 = linkList.length; i < length1; i++) {
    ...
    if (attrs.rel && attrs.rel.toLowerCase() === 'stylesheet') {
      if (options.applyLinkTags) results.push({ href: attrs.href, link: link });
...
function extractCssFromDocument(document, options, callback) {
  ...
  batch.push(function(callback) { 
    getStylesData(document, options, callback);
  });
  getStylesheetList(document, options).forEach(function (stylesheet) {
    batch.push(function(callback) {
      getHrefContent(stylesheet.href, options.url, function (err, css) {
        callback(err, { css: css, link: stylesheet.link });
      });
    });
  });
  batch.end(function (err, results) {
    ...
    results.forEach(function(stylesheet) {
