...
cli.getProgram = function(argv) {
  ...
  program
    ...
    .version(pkg.version)
    .argument('[input.html]', 'input HTML file or directory, or - for stdin (default when piped)')
    .argument('[output.html]', 'output HTML file or directory, or - for stdout (default)');
...
cli.options = {
  ...
  'web-resources-strict': {
    ...
    coercion: JSON.parse },
  'add-important-to-pseudo-classes': {
    pMap: 'addImportantToPseudoClasses',
    def: 'add !important to declarations in pseudo-class rules like :hover?',
    coercion: JSON.parse },
...
cli.run = function(argv, deps) {
  ...
  if (program.args.length === 0 && stdin.isTTY) {
    program.help();
    return;
  }

  const [inputFile, outputFile] = program.args;
  const fromStdin = !inputFile || inputFile === '-';
  ...
  const readStdin = (callback) => {
    ...
    stdin.on('end', () => callback(null, Buffer.concat(chunks).toString('utf8')));
  };

  const juiceDirectory = () => {
    if (toStdout) {
      handleError(new Error('When the input is a directory, pass an output directory too.'));
      return;
    }

    const inputDir = path.resolve(cwd, inputFile);
    const outputDir = path.resolve(cwd, outputFile);
    const isInOutputDir = (file) => file === outputDir || file.startsWith(outputDir + path.sep);
    const files = fs.readdirSync(inputDir, { recursive: true })
      .filter((file) => /\.html?$/i.test(file) && !isInOutputDir(path.join(inputDir, file)))
      .sort();
    ...
    const next = (index) => {
      if (index === files.length) {
        if (failures) {
          exit(1);
          done(new Error(failures + ' of ' + files.length + ' files failed'));
        } else {
          done();
        }
        return;
      ...
      const fileDone = (err) => {
        if (err) {
          failures++;
          error(path.join(inputFile, file) + ': ' + err.message);
        }
        next(index + 1);
      };

      // juiceFile fills in webResources.relativeTo per file, so each file needs its own copy.
      const fileOptions = Object.assign({}, options, { webResources: Object.assign({}, options.webResources) });
      juice.juiceFile(path.join(inputDir, file), fileOptions, (err, html) => {
        if (err) {
          fileDone(err);
          return;
        }
        ...
        fsImpl.writeFile(target, html, fileDone);
      });
    };
    next(0);
  };
  ...
  const finish = () => {
    ...
    if (!fromStdin && fs.statSync(path.resolve(cwd, inputFile), { throwIfNoEntry: false })?.isDirectory()) {
      juiceDirectory();
      return;
    }

    if (!fromStdin) {
      juice.juiceFile(inputFile, options, write);
      return;
    }
    ...
    readStdin((err, html) => {
      ...
      if (options.codeBlocks) {
        Object.keys(options.codeBlocks).forEach((key) => {
          juice.codeBlocks[key] = options.codeBlocks[key];
        });
      }
      juice.juiceResources(html, options, write);
    });
  };
